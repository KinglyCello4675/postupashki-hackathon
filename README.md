...
**`docs/`** — документация:
- `solution.pdf` — итоговое решение, 2 страницы
- `presentation.pdf` — презентация, 10 слайдов
- `spec.md` — модель данных и спецификация атрибуции
- `history.md` — выводы исторического анализа
- `architecture.md` — схема системы и user stitching
- `tracking.md` — реестр размещений и source
...
Главный вывод проекта: рост продаж можно получить не закупкой рекламы, а продуктом и календарём.

Анализ третьего всплеска (04–06.09) показал:

- всплеск дал не общий рост, а конкретную линейку — ПРО. Её доля в выручке выросла с 35% до 55% за неделю;
- дедлайн акции совпал с дедлайном подачи заявок на стажировку в Т-Банке. После дедлайна продажи упали на 67% — спрос не исчез, а был стянут к дате;
- новый продукт (AI агенты) стал первым курсом каталога по выручке в день — 37 456 ₽, хотя продавался всего 19 дней.

Две стратегии, которые работают без рекламного бюджета:

1. Привязка к дедлайнам отбора в бигтех. Календарь найма публичен и известен заранее. Запуски и промо концентрируются за 1–2 недели до дедлайнов.
2. Запуск новых направлений. Новый продукт даёт высокую скорость выручки, а не только пик на существующем бренде.

Дополнительно: партнёрство с работодателем вместо закупки рекламы. Бартер «постоянное размещение партнёра в канале ↔ гарантированный технический собес для топ-N выпускников ПРО». Расход бюджета нулевой, а ценность продукта растёт.

Если всё же тратить 300 000 ₽ на рекламу:

1. Сначала — измеряемость: tracking-ссылки на каждое размещение, реестр с cost, логирование source у менеджера.
2. Бюджет тремя корзинами: ~50% на проверенное (дедлайны, акции), ~30% на тесты новых каналов, ~20% на holdout и randomized promotion.
3. Порог CPM по каждому каналу — прайс сравнивается с ним до закупки, а не после.
4. Успех — не ROMI_attr, а ROMI_inc из эксперимента. Если он близок к нулю, честный вывод: деньги лучше вложить в продукт и календарь запусков.

...
- исключение `is_repeat = true` из ROMI;
- отдельная строка `repeat_customer` без ROMI;
- проверка равенства суммы покупок и суммы выручки в итоговом CSV;
...
- сгенерировано `500` покупок, `153` касания и `10` размещений;
- `133` пользователя имеют рекламные касания;
- demo-выручка: `3 534 704,67 ₽`;
- в режиме `linear` обработано `500` покупок;
- повторная выручка `357 888,34 ₽` вынесена в `repeat_customer`;
- общая demo-выручка сохранена: сумма итогового CSV равна сумме покупок;
- значение `-1` в итоговом ROMI отсутствует.

В режиме `linear` актуальные значения ROMI платных размещений находятся
в диапазоне `71,71–253,21%`. Строка `repeat_customer` имеет `cost = 0`
и пустой ROMI.
...
Текущий набор файлов был удалён и заново сгенерирован перед обновлением
этого отчёта.
...
После запуска в `romi_demo.csv` будут отдельные строки `organic` и
`repeat_customer`, если такие записи есть. Повторные покупки не участвуют
в ROMI платных размещений, но сохраняются в общей выручке.
